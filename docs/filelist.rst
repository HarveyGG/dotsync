...
Your filelist is where all the magic happens, and correctly using dotsync's
...
Along with dotsync's automatic hostname category (see the :doc:`usage` section
...
Plugins allow you to go beyond dotsync's normal symlinking of dotfiles.
Currently dotsync only has one plugin named ``encrypt``, which allows you to
